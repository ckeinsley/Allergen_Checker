...
# Resolve the SQLite DB path relative to this file so it works in dev and installed environments
DB_PATH = (Path(__file__).resolve().parent / 'database_files' / 'allergens.sqlite')
...
class CommonAllergen(Base):
    ...
    scientific_allergens = relationship(
        "ScientificAllergen", back_populates="common_allergen")
...
class ScientificAllergen(Base):
    ...
    common_allergen = relationship(
        "CommonAllergen", back_populates="scientific_allergens")
...
class SqlLiteDatabase(Database):
    ...
    def check_word(self, session, word) -> CheckedWord:
        ...
        if common:
            scientific_names = [
                scientific.name for scientific in common.scientific_allergens]
            return CheckedWord(
                checked_word=word,
                match=Allergen(
        ...
        if scientific:
            common = self.check_common_exists(
                session, scientific.common_allergen.name)
            scientific_names = [
                scientific.name for scientific in common.scientific_allergens]
            return CheckedWord(
                checked_word=word,
    ...
    def get_all_allergens(self, skip, limit) -> List[Allergen]:
        session = self.connect_to_db()
        common_allergens = session.query(CommonAllergen).order_by(
            CommonAllergen.name).offset(skip).limit(limit).all()
        results = []
        for common_allergen in common_allergens:
            scientific_allergens = (
                session.query(ScientificAllergen)
                .filter(ScientificAllergen.common_allergen_id == common_allergen.id)
                .order_by(ScientificAllergen.name)
                .all()
            )
            result_allergen = Allergen(
                common_name=common_allergen.name,
                scientific_names=[scientific.name for scientific in scientific_allergens],
            )
    ...
    def add_allergens(self, allergens: List[Allergen]):
        '''
        Adds allergens to the database. If the common exists already, just append
        scientific to it; otherwise both are created.
        '''
        session = self.connect_to_db()
        for allergen in allergens:
            ...
            for scentific_allergen in allergen.scientific_names:
                scientific = ScientificAllergen(
                    name=scentific_allergen.lower(), common_allergen=common)
    ...
    def delete_allergens(self, allergens: List[Allergen]):
        '''
        If common name, delete everything. Else only remove scientific. If the last
        scientific is deleted, delete everything.
        '''
        session = self.connect_to_db()
        for allergen in allergens:
            common_model = self.check_common_exists(
                session, allergen.common_name)
            if not common_model:
                logging.info("Attempted to delete something that doesn't exist")
                break
            for scientific_name in allergen.scientific_names:
                scientific_model = (
                    session.query(ScientificAllergen)
                    .filter(
                        ScientificAllergen.name == scientific_name.lower(),
                        ScientificAllergen.common_allergen == common_model,
                    )
                    .first()
                )
                if scientific_model:
    ...
    def check_common_exists(self, session, name):
        existing_common_allergen = session.query(
            CommonAllergen).filter_by(name=name.lower()).first()
        return existing_common_allergen

    def check_scientific_exists(self, session, name):
        existing_scientific_allergen = session.query(
            ScientificAllergen).filter_by(name=name.lower()).first()
        return existing_scientific_allergen

    def check_scientific_exists_fuzzy(self, session, name):
        existing_scientific_allergen = session.query(ScientificAllergen).filter(
            ScientificAllergen.name.like(f'%{name.lower()}%')).first()
        return existing_scientific_allergen
